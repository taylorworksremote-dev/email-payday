...
python scripts/classify_and_report.py --input gmail_raw.json icloud_raw.json --out-prefix payday_scan --tz America/New_York
```
...
This produces `payday_scan.csv` and `payday_scan.xlsx`. Gmail collection itself (`gmail_raw.json`) is driven by Claude through the Gmail connector per the steps in `SKILL.md` — there's no standalone Gmail script here because it relies on Claude's own connector tools rather than direct API calls.
