...
def write_outputs(rows, out_prefix):
    import csv
    columns = ["date_local_str", "time_local_str", "account", "folder", "sender_name",
               "sender_email", "company_guess", "subject", "matched_keywords",
               "excerpt", "source_ref", "screenshot_path"]
    ...
    # Only potential FDCPA violations ever reach this function (main() filters out
    # everything else) -- so there's a single sheet, and every row on it is a hit.
    headers = ["Date (Local)", "Time (Local)", "Account", "Folder", "Sender Name",
               "Sender Email", "Company (guess)", "Subject", "Matched Keyword(s)",
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Potential FDCPA Claims"
    bold = Font(bold=True)
    ws.append(headers)
    for cell in ws[1]:
        cell.font = bold
    ...
    for r in rows:
        vals = [r["date_local_str"], r["time_local_str"], r["account"], r["folder"],
                r["sender_name"], r["sender_email"], r["company_guess"], r["subject"],
                r["matched_keywords"], r["excerpt"], r["source_ref"],
...
def main():
    ...
    ap.add_argument("--screenshots", choices=["all", "none"], default="all",
                     help="Only potential-FDCPA (off-hours) rows ever make it into the report "
                          "(see main()), so 'all' (default) screenshots every row that's "
                          "actually output; 'none' skips screenshots entirely.")
    ...
    seen, rows = set(), []
    dropped_old = dropped_no_match = dropped_bad_date = dropped_not_flagged = 0
    ...
    for r in raw_records:
        ...
        if not is_flagged:
            # Only actual potential-FDCPA (off-hours) hits are worth a human's time --
            # everything else was just noise from the deliberately broad keyword list.
            dropped_not_flagged += 1
            continue

        date_local_str = dt_local.strftime("%m/%d/%Y")
        time_local_str = dt_local.strftime("%I:%M %p %Z")
        ...
        if args.screenshots != "none":
            ...
            screenshot_path = render_email_screenshot(
                ...
                r.get("sender_name", ""),
                r.get("sender_email", ""),
                f"{date_local_str} {time_local_str}",
                body_html,
                body,
                candidate_path,
            )
    ...
    print(f"  Dropped (not a potential FDCPA violation -- arrived in-hours): {dropped_not_flagged}", file=sys.stderr)
    print(f"  Potential FDCPA violations (final report rows): {len(rows)}", file=sys.stderr)
    if args.screenshots != "none":
